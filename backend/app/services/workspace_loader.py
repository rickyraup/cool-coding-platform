...
class WorkspaceLoaderService:
    ...
    async def load_workspace_into_container(self, session_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to load workspace for session {session_id}: {e}")
            return False
    
    async def _create_workspace_structure(self, workspace_items: List[WorkspaceItem], base_dir: str):
    ...
    async def save_workspace_from_container(self, session_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to save workspace for session {session_id}: {e}")
            return False
    
    async def _scan_and_save_workspace(self, session_id: int, base_dir: str, parent_id: Optional[int] = None, current_path: str = ""):
        ...
        try:
            ...
            for item_name in os.listdir(base_dir):
                ...
                relative_path = os.path.join(current_path, item_name) if current_path else item_name
            ...
            logger.error(f"Failed to scan directory {base_dir}: {e}")
            raise
    
    async def get_workspace_file_content(self, session_id: int, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Failed to read file {file_path} from session {session_id}: {e}")
            return None
    
    async def update_workspace_file_content(self, session_id: int, file_path: str, content: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to update file {file_path} in session {session_id}: {e}")
            return False
