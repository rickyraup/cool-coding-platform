...
class DockerClientService:
        ...
        if self._client is None:
            try:
                ...
                connection_methods = [
                    ...
                    lambda: docker.DockerClient(base_url=f'unix:///Users/{user}/Library/Containers/com.docker.docker/Data/docker-cli.sock'),
                    # Alternative Docker Desktop socket
                    lambda: docker.DockerClient(base_url=f'unix:///Users/{user}/.docker/run/docker.sock'),
                ...
                if self._client is None:
                    raise ConnectionError(f"Cannot connect to Docker daemon with any method. Last error: {last_error}")
    ...
    def create_session_container(self, session_id: str, working_dir: str) -> Container:
        ...
        if not self.ensure_image_exists():
            raise RuntimeError(f"Docker image {self._image_name} not available. Please build the image first.")
        ...
        try:
            ...
            logger.info(f"Created container {container.short_id} for session {session_id}")
            return container
    ...
    def stop_container(self, container: Container, timeout: int = 10) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to stop container {container.short_id}: {e}")
            try:
                container.kill()
                logger.info(f"Force killed container {container.short_id}")
                return True
            except Exception as kill_error:
                logger.error(f"Failed to kill container {container.short_id}: {kill_error}")
                return False
