...
class SessionManager:
    ...
    async def create_session(self, session_id: str) -> SessionProcess:
        ...
        session = SessionProcess(
            session_id=session_id, process=process, working_directory=session_dir,
        )
    ...
    async def _handle_ps_command(self, session: SessionProcess) -> tuple[str, int]:
        ...
        try:
            ...
            child_output, _ = await self._execute_command_in_session(
                session, pgrep_command,
            )

            # Build session-specific process list
            session_output = (
                f"SESSION PROCESSES (Isolated Session: {session.session_id[:8]})\n"
            )
            session_output += "  PID  PPID TTY      TIME CMD\n"
            session_output += (
                f"  {shell_pid}     1 pts/0    00:00:01 bash (session shell)\n"
            )
            ...
            if child_output.strip() and child_output.strip() != "no_children":
                ...
                for child_pid in child_pids:
                    if child_pid.strip().isdigit():
                        ...
                        child_info, _ = await self._execute_command_in_session(
                            session, ps_child,
                        )
                        if child_info.strip():
                ...
                session_output += (
                    "  (No additional processes - commands will spawn here)\n"
                )
            ...
            return f"Error getting session processes: {e!s}", 1
    ...
    async def _handle_id_command(self, session: SessionProcess) -> tuple[str, int]:
        ...
        session_uid = (
            hash(session.session_id) % 50000 + 1000
        )  # Generate consistent fake UID
        return (
            f"uid={session_uid}(session-user-{session.session_id[:8]}) gid={session_uid}(session-group) groups={session_uid}(session-group)",
            0,
        )

    async def _execute_command_in_session(
        self, session: SessionProcess, command: str,
    ) -> tuple[str, int]:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    if (
                        not line_str.startswith("+") and line_str.strip()
            ...
            return f"Error executing command: {e!s}", 1
