...
async def create_workspace_item(item_data: WorkspaceItemCreate) -> WorkspaceItemDetailResponse:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to create workspace item: {e!s}",
        )


@router.get("/{item_id}")
async def get_workspace_item(item_id: int) -> WorkspaceItemDetailResponse:
    """Get a specific workspace item."""
    try:
        item = WorkspaceItem.get_by_id(item_id)
        if not item:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Workspace item not found",
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve workspace item: {e!s}",
        )


@router.put("/{item_id}")
async def update_workspace_item(item_id: int, item_update: WorkspaceItemUpdate) -> WorkspaceItemDetailResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to update workspace item: {e!s}",
        )


@router.delete("/{item_id}")
async def delete_workspace_item(item_id: int) -> BaseResponse:
    ...
    try:
        ...
        return BaseResponse(success=True, message="Workspace item deleted successfully")
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to delete workspace item: {e!s}",
        )


@router.get("/session/{session_id}/tree")
async def get_session_workspace_tree(session_id: int) -> WorkspaceTreeResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve workspace tree: {e!s}",
        )


@router.get("/session/{session_id}")
async def get_session_workspace_items(session_id: int, parent_id: Optional[int] = None) -> WorkspaceItemListResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve workspace items: {e!s}",
        )
