...
def build_workspace_tree(items: List[WorkspaceItem], parent_id: Optional[int] = None) -> List[WorkspaceTreeItem]:
    ...
    for item in items:
        if item.parent_id == parent_id:
            tree_item = WorkspaceTreeItem(
                ...
                type=item.type,
                full_path=item.get_full_path(),
                children=build_workspace_tree(items, item.id) if item.type == 'folder' else None
            )
...
@router.get("/{session_id}/workspace", response_model=SessionWithWorkspaceResponse)
async def get_session_with_workspace(session_id: int):
    ...
    try:
        ...
        session_response = convert_session_to_response(session)
        workspace_responses = [convert_workspace_item_to_response(item) for item in workspace_items]
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve session workspace: {str(e)}"
        )